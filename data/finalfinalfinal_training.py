...
with Step("Load PHARMACY (CSV or NPI Excel bundle)"):
    # Try directory of Excel files first, then CSV
    excel_dir = Path("data/Pharmacy_list_ZIP_fixed_final")
    csv_path = Path("data/pharmacy_data.csv")
    
    ph = None
    
    if excel_dir.exists() and excel_dir.is_dir():
        stamp(f"Using Excel directory pharmacy data: {excel_dir}")
        import openpyxl
        subset_files = sorted(excel_dir.glob("subset*_Table1_filter.xlsm"))
        ...
        if subset_files:
            ...
            for file in subset_files:
                try:
                    ...
                    if 'Provider Organization Name (Legal Business Name)' in df_subset.columns:
                        df_extracted['NAME'] = df_subset['Provider Organization Name (Legal Business Name)']
                    if 'Provider Business Practice Location Address State Name' in df_subset.columns:
                        df_extracted['STATE'] = df_subset['Provider Business Practice Location Address State Name']
                    if 'Short_ZIP' in df_subset.columns:
                        df_extracted['ZIP'] = df_subset['Short_ZIP']
                    
                    if not df_extracted.empty and 'ZIP' in df_extracted.columns:
                        all_pharm_data.append(df_extracted)
                    
                except Exception as e:
                    stamp(f"Warning: Could not read {file.name}: {e}")
                    continue
            
            if all_pharm_data:
                ph = pd.concat(all_pharm_data, ignore_index=True)
                stamp(f"Loaded {len(ph):,} pharmacy records from {len(subset_files)} Excel files")
    
    if ph is None and csv_path.exists():
